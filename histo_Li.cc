  ...
  for(int om = 0; om < 712; om+=1)
  {
    for (double j = debut; j < debut+6*40.5; j = j+40.5)
    {
      ...
      if (spectre->GetEntries() < 300) {
        ...
        if (f_Gaus->GetChisquare()/f_Gaus->GetNDF() > 2 || f_Gaus->GetChisquare()/f_Gaus->GetNDF() < 0.25) {
          outFile << "om = " << om << " and pic = " << pic << " and Khi2 = "<< f_Gaus->GetChisquare()/f_Gaus->GetNDF() <<  endl;
        }
...
void fit_LI_ampl(){
  ...
  TFile file("Resultats_root/Resultat_charge_Li_1.root","RECREATE");
...
void om_linearity(string file) {
  TFile result_file(Form("Resultats_root/Resultat_%s.root",file),"RECREATE");
...
//         for (double j = debut; j < debut+5*40.5; j+=40.5)
//         {
//           TF1* f_ComptonEdgePoly = new TF1 ("f_ComptonEdgePoly","[0]/2.0*(1+TMath::Erf(([1]-x)/(TMath::Sqrt(2)*[2])))+[3]*x", 40000, 90000);
//           if ((om > 259 && om < 520) || (om > 583 && om < 647) || (om > 679)) {temps+=(265+j-17);}
